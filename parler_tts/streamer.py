...
class AsyncParlerTTSStreamer(BaseStreamer):
    def __init__(
        self,
        model: ParlerTTSForConditionalGeneration,
        device: Optional[str] = None,
    ...
    def decode_audio(self, input_ids):
    ...
    def apply_delay_pattern_mask(self, input_ids):
        ...
        if not decode_sequentially:
            output_values = self.decode_audio(input_ids)
            sample = self.audio_encoder.decode(
                audio_codes=input_ids,
                **self.audio_kwargs,
            ).audio_values
            ...
            sample_mask = ((sample >= self.audio_encoder.config.codebook_size).sum(dim=(0, 1)) == 0) if self.use_4dim_audio_codes else ((sample >= self.audio_encoder.config.codebook_size).sum(dim=0) == 0)
            sample = sample[:, :, sample_mask] if self.use_4dim_audio_codes else sample[:, sample_mask]
            output_values = self.decode_audio(sample[None, ...])
    ...
    def put(self, value):
        ...
        if self.token_cache.shape[-1] % self.play_steps == 0:
            ...
            if self.to_yield == 0:
                self.to_yield = len(audio_values) - self.to_yield - self.stride
            self.token_cache = self.token_cache[...,-self.play_steps:]
    ...
    def on_finalized_audio(self, audio: np.ndarray, stream_end: bool = False):
        ...
        if stream_end:
            self.loop.call_soon_threadsafe(self.audio_queue.put_nowait, self.stop_signal)
        ...
        try:
            if self.has_asyncio_timeout:
                async with asyncio.timeout(self.timeout):
                    value = await self.audio_queue.get()
            else:
                value = await asyncio.wait_for(self.audio_queue.get(), timeout=self.timeout)
